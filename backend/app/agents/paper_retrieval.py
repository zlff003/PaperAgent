...
class PaperRetrievalAgent:
    ...
    def search(
        ...
        if query and query.strip():
            hits = vector_store.query(query.strip(), top_k=top_k)
            for hit in hits:
    ...
    async def search_async(
        ...
        """Run semantic and metadata searches in parallel, then merge."""
        ...
        async def semantic() -> list[dict[str, Any]]:
            return await asyncio.to_thread(vector_store.query, query.strip(), top_k)
